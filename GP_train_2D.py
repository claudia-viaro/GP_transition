...
class GaussianProcessClassifierLaplace(object):
    ...
    def fit(self, X, y):
        ...
        if self.optimizer is not None and self.kernel_.n_dims > 0:
            ...
            print("self.kernel_.bounds", self.kernel_.bounds)
    ...
    def J_function(self, newXa, eval_gradient=True):
        # Compute function J
    ...
    def _constrained_optimization(self, obj_func, initial_theta, bounds):
        if self.optimizer == "fmin_l_bfgs_b":
            opt_res = minimize(
                obj_func, initial_theta, method="L-BFGS-B", jac=True, bounds=bounds, options={'maxiter': 5000000, 'gtol':1e-06}
            )
    ...
    def derivate_kern(self, testX):
        ...
        if self.kernel_options == 'polynomial':
            ...
            for i in range(self.X_train_.shape[0]):
                der1[i] = self.exponent*self.X_train_[i]
                der2[i] = 2*self.exponent*self.X_train_[i]
            der_tuple = [np.array(der1*np.inner(self.X_train_, testX) **(self.exponent - 1)).squeeze(), np.array(der2*np.inner(self.X_train_, testX) **(2*self.exponent - 1) ).squeeze()]  
    ...
    def derivate_kern_i(self, testX):
        ...
        if self.kernel_options == 'rbf':
            ...
            gamma = 1/(2*(self.kernel_.theta[0]**2))                  
        ...
        if self.kernel_options == 'polynomial':        
            ...
            der2 = [0]*self.X_train_.shape[0]
            for i in range(self.X_train_.shape[0]):
                der1[i] = self.exponent*self.X_train_[i]*self.kernel(testX[i])**(self.exponent - 1)
                der2[i] = 2*self.exponent*self.X_train_[i]*self.kernel(testX[i])**(2*self.exponent - 1) 
            der_tuple = [np.array(der1).squeeze(), np.array(der2).squeeze()]  
    ...
    # Xtest is a single instance of the vector
        ...
        t = t1* t2*t3
        #print("der shape", self.derivate_kern(Xtest)[1])
